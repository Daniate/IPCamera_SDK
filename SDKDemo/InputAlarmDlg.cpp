...
void CInputAlarmDlg::OnBtnSnap() 
{
	CSDKDemoDlg *pDlg = (CSDKDemoDlg *)AfxGetMainWnd();
	char strPath[MAX_PATH] = {0};

	HI_S_ExtAlarm m_ExtAlarm;
	HI_S32 s32Ret = HI_SDK_SetConfig(m_lHandle, HI_CMD_EXT_ALARM, &m_ExtAlarm, sizeof(HI_S_ExtAlarm));
	if(s32Ret < HI_SUCCESS)
	{
		MessageBox("手动触发外置报警失败!\r\n");
		return;
	}
}
