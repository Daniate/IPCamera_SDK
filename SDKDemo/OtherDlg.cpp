...
BEGIN_MESSAGE_MAP(COtherDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_CK_RELAYCTRL, OnCkRelayctrl)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void COtherDlg::OnCkRelayctrl() 
{
	HI_S32 s32Ret = HI_FAILURE;
	HI_S_RelayCtrl RelayCtrl;
	...
	memset(&RelayCtrl, 0, sizeof(HI_S_RelayCtrl));
	if(((CButton *)GetDlgItem(IDC_CK_RELAYCTRL))->GetCheck())
		RelayCtrl.bEnable = HI_TRUE;
	else
		RelayCtrl.bEnable = HI_FALSE;

	s32Ret = HI_SDK_SetConfig(m_lHandle, HI_CMD_RELAYCTRL, &RelayCtrl, sizeof(HI_S_RelayCtrl));
	if(HI_SUCCESS != s32Ret)
		MessageBox(ConvertString("Setting Failure!"));
