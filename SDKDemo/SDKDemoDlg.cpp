...
HRESULT HISDK_CALLBACK MyPostDrawCallBackEx(HI_VOID * lRes,	HI_VOID *hDc,HI_S32 s32ImageWidth,
			HI_S32 s32ImageHeight,HI_S32 s32WndWidth,HI_S32 s32WndHeight,HI_S32 s32Offx,HI_S32 s32Offy,HI_U64 u64TimeStamp,HI_VOID *pPara);
...
void CSDKDemoDlg::OnBtnConnect() 
{
	...
	s32Ret = HI_SDK_SetPostDrawCallBackEx(lHandle,(HI_U32)lHandle,MyPostDrawCallBackEx,this);
...
int CSDKDemoDlg::Snap_Bmp()
{
	...
	CString csFileName = CTime::GetCurrentTime().Format("SnapRt_%y_%m_%d_%H_%M_%S.bmp");
	char strPath[256];
...
int Save_File(const char* filename,const unsigned char* pData,int nLen)
{

	if(pData == NULL || filename == NULL || nLen <= 0)
		return -1;
	...
	fwrite((const char*)pData, 1, nLen, fp);
	fclose( fp );

	return 0;
}

int CSDKDemoDlg::Snap_YUV420()
{
	if(0 == m_sCamInfo[m_u32CurScr].lHandle)
	{
		MessageBox(ConvertString("No connection!"));
	...
	CString csFileName = CTime::GetCurrentTime().Format("SnapRt_%y_%m_%d_%H_%M_%S.yuv");
	...
	if(yuv.pData)
	{
		if((s32Ret =HI_SDK_SnapYUVData(m_sCamInfo[m_u32CurScr].lHandle,&yuv,0,&nNeedSize)) == HI_SUCCESS)
		{
			//yuv
			Save_File(strPath,yuv.pData,nNeedSize);
		}
...
void CSDKDemoDlg::OnBtnPhoto() 
{
	...
	TRACE("snap picture return value:%d\n",nRet);
}
...
HI_S32 CSDKDemoDlg::funDevRProc(const HI_VOID* pvHandle,
								  HI_CHAR* pszRNPCode,
...
void CSDKDemoDlg::OnBtnSearch() 
{
	...
	if(NULL == m_pScHandle)
	{
		s32Ret = HI_VSCP_DEVSCLI_INIT(SEARCH_BROADIP, SEARCH_BROADPORT, 2, &m_pScHandle);
...
HRESULT HISDK_CALLBACK MyPostDrawCallBackEx(HI_VOID * lRes,	HI_VOID *hDc,HI_S32 s32ImageWidth,
			HI_S32 s32ImageHeight,HI_S32 s32WndWidth,HI_S32 s32WndHeight,HI_S32 s32Offx,HI_S32 s32Offy,HI_U64 u64TimeStamp,HI_VOID *pPara)
 {
	...
	char * pText = "This is Ownerdaw demo!";

	int h = 50;
	HDC *phDc = (HDC*)hDc;
	
	CFont cFont;
	cFont.CreateFont(h, 0, 0, 0,
		800, (HI_U8)HI_FALSE, (HI_U8)HI_FALSE,
		0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
		DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, "Arial");
	
	SelectObject(*phDc, cFont);
	SetBkMode(*phDc, TRANSPARENT);
	SetBkColor(*phDc, RGB(255, 0, 0));
	SetTextColor(*phDc, RGB(0, 255, 0));
	
	
	TextOut(*phDc,  100,100, pText, strlen(pText));
