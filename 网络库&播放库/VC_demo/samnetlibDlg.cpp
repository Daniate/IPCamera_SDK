...
HRESULT HSPLAYER_APICALL My_PostDrawCallBackEx(PLAYHANDLE hHandle,
		HI_VOID *hDc,
        ...
        HI_VOID *pPara)
{
	...
	int h = 50;
	char * pText = "This is Ownerdraw Demo!";
	//::SetBkMode( *phDc,TRANSPARENT );
	SetBkColor( *phDc, RGB(128,128,128) );
...
/*
	...
	gPen.CreatePen(PS_SOLID, nPenWidth, RGB(0, 0, 255));
	oldPen = pDC->SelectObject(&gPen);
...
void CSamnetlibDlg::MyPlay() 
{
	if(NULL == m_hPlay)
	{
		...
		PLAYER_DISPLAYMODE_E disMode;
		disMode = PLAYER_DPY_D3D;
		HI_PLAYER_SetDisplayMode(m_hPlay,disMode);
		HI_PLAYER_Play(m_hPlay);
		HI_PLAYER_GetDisplayMode(m_hPlay, &disMode);
		HI_PLAYER_SetDrawCallBack(m_hPlay, HI_TRUE);
//		HI_PLAYER_SetDrawCallBack(m_hPlay, HI_FALSE);
		HI_PLAYER_SetPostDrawCallBackEx(m_hPlay,My_PostDrawCallBackEx,this);
//		HI_PLAYER_SetPostDrawCallBack(m_hPlay, My_PostDrawCallBack, this);
